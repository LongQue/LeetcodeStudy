...
func findInMountainArray(target int, mountainArr *MountainArray) int {
	//找出最高峰，分两段找山峰，先找左边，如果左边没有相等的，则可能存在右边
	...
	lIdx := binSearch(mountainArr, 0, peakIdx, target, true)
	if lIdx == -1 {
		return binSearch(mountainArr, peakIdx+1, mountainArr.length()-1, target, false)
	}
	return lIdx

}
func binSearch(mountainArr *MountainArray, l, r, target int, asc bool) int {
	for l <= r {
		mid := l + (r-l)/2
		midValue := mountainArr.get(mid)
		if midValue == target {
